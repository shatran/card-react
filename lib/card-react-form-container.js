(function() {
  ...
  CardReactFormContainer = React.createClass({
    ...
    traverseChildrenAndRegisterInputs: function(children) {
      ...
      return React.Children.map(children, (function(_this) {
        return function(child) {
          var inputAttribute, inputsValidationClass, newClassName;
          if (typeof child !== 'object' || child === null) {
            return child;
          }
          inputAttribute = child.props[_this.props.inputAttributeIndentifier];
          if (child.props && inputAttribute) {
            newClassName = child.props.className;
            inputsValidationClass = _this.state.inputsValidationClass[inputAttribute];
            if (child.ref) {
              _this.inputsRefs[inputAttribute] = child.ref;
            } else {
              _this.inputsRefs[inputAttribute] = "react-card-input-" + inputAttribute;
            }
            ...
            return React.cloneElement(child, {
              ...
              ref: _this.inputsRefs[inputAttribute],
              defaultValue: _this.inputsValues[inputAttribute],
    ...
    inputOnKeyUp: function(event) {
      this.input = event.target[this.props.inputAttributeIndentifier];
      this.inputsValues[this.input] = event.target.value;
      this.validateInput(this.input, event.target.value);
      return this.renderCardComponent();
    },
    inputOnFocus: function(event) {
      this.focusedInput = event.target[this.props.inputAttributeIndentifier];
