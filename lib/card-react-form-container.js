(function () {
  var CardReactFormContainer, Payment, ClassNames, React, ReactCard, ReactDOM, createReactClass;
  ...
  CardReactFormContainer = createReactClass({
      ...
      return React.Children.map(children, (function (_this) {
        return function (child) {
          ...
          if (child.props && child.props.name) {
            ...
            var bootstrapValidationClass;

            if (_this.props.bootstrapValidator) {
              if (child.props.className) {
                if (child.props.className.indexOf("form-group") !== -1) {

                  React.Children.map(child.props.children, function (childChildren) {
                    if (childChildren.props && childChildren.props.name) {
                      bootstrapValidationClass = _this.state.inputsValidationClass[childChildren.props.name];
                    }
                  });
            ...
            return React.cloneElement(child, {
              className: ClassNames(child.props.className, bootstrapValidationClass || '')
            }, _this.traverseChildrenAndRegisterInputs(child.props && child.props.children));
          }
        };
      })(this));
    },
    inputOnKeyUp: function (event) {
      ...
      if (this.focusedInput === this.props.formInputsNames['number']) {
        this.inputsValues[this.props.formInputsNames['typeCard']] = Payment.fns.cardType(event.target.value) || '';
      }
